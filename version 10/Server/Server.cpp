...
struct SocketInfo
{
	...
	unsigned short state; //1:대기, 2:방 내부
};
struct Room
{
	int roomNum;
	...
	while (1) {
		...
		if (retval == SOCKET_ERROR) {
			...
			case 'J': //지정한 방에 입장 하라
			{
				...
				SocketInfo& sockInfo = sockList[idx];
				if (sockInfo.state == 1)
				{
					sockInfo.state = 2;

					int roomNum = *(int*)(buf + HEADSIZE);
					roomGroup[roomNum].sockList.push_back(sockInfo);
			...
			case 'R': //방의 정보 달라
			{
				*(int*)(buf + HEADSIZE) = roomGroup.size() - 1;
				send(client_sock, buf, BUFSIZE, 0);
			}
			break;
			case 'M': //방을 생성 하라
			{				
				int idx = FindSocket(roomGroup[0].sockList, client_sock);
				SocketInfo& sockInfo = roomGroup[0].sockList[idx];
				if (sockInfo.state == 1)// 대기상태: 이미 방에 입장한 클라이언트는 방 개설 불가
				{
					sockInfo.state = 2; // 방 입장 상태 설정
					roomGroup.push_back(Room());
					roomGroup[roomGroup.size() - 1].
						sockList.push_back(sockInfo);
					for (unsigned i = 1; i < roomGroup[0].sockList.size(); ++i)
					{
						//////////방 정보 생성
					}
					*(int*)(buf+HEADSIZE) = roomGroup.size()-1;
			...
			case 'S':
			{
				...
				{
					int roomNum = i;
					vector<SocketInfo>& sockList = roomGroup[roomNum].sockList;
					int idxFind = FindSocket(sockList, client_sock);
					if (idxFind >= 0)
					{
						for (unsigned i = 0; i < sockList.size(); ++i)
						{
